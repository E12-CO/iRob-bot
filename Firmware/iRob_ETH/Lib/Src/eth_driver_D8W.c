...
#include "eth_driver.h"
 ...
 __attribute__((aligned(4))) ETH_DMADESCTypeDef DMARxDscrTab[ETH_RXBUFNB];      /* MAC receive descriptor, 4-byte aligned*/
 __attribute__((aligned(4))) ETH_DMADESCTypeDef DMATxDscrTab[ETH_TXBUFNB];      /* MAC send descriptor, 4-byte aligned */

 __attribute__((aligned(4))) uint8_t  MACRxBuf[ETH_RXBUFNB*ETH_RX_BUF_SZE];     /* MAC receive buffer, 4-byte aligned */
...
const uint16_t MemSize[8] = {WCHNET_MEM_ALIGN_SIZE(WCHNET_SIZE_IPRAW_PCB),
 ...
 __attribute__((aligned(4)))uint8_t Memp_Memory[WCHNET_MEMP_SIZE];
 __attribute__((aligned(4)))uint8_t Mem_Heap_Memory[WCHNET_RAM_HEAP_SIZE];
 __attribute__((aligned(4)))uint8_t Mem_ArpTable[WCHNET_RAM_ARP_TABLE_SIZE];
 ...
 *
 * @brief   Get MAC address from the Unique ID section
 *
 * @return  none.
 */
 ...
 * @fn      WCHNET_TimeIsr
 *
 * @brief	Update time tick for Ethernet Link maintaining 
 *
 * @return  none.
 */
...
void WCHNET_LinkProcess( void )
{
    ...
    if(phy_anlpar&PHY_ANLPAR_SELECTOR_FIELD)
    {
        if( !(phyLinkStatus&PHY_LINK_WAIT_SUC) )
        {
			...
			// Toggle the polarity switch back and forth 
 ...
 * @fn      ETH_PHYLink
 *
 * @brief	Ethernet Link maintaining
 *
 * @return  none
 */
 ...
 * @fn      WCHNET_ETHIsr
 *
 * @brief	ISR handing the Ethernet interrupt
 *
 * @return  none
 */
